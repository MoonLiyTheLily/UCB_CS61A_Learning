...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        if self.free_withdrawals:
            ...
            if amount>self.balance:
                return "Insufficient funds"
            #What about that max_withdraw?I havent implement it.
            self.balance=self.balance-amount
            return self.balance
        else:
...
def without(s, i):
    ...
    """
    "*** YOUR CODE HERE ***"
    s_copy=Link(s.first,s.rest)#make a copy first
    if i==0:
        return s_copy.rest
    if s_copy.rest==Link.empty:
        return s_copy
    else:
        s_copy.rest=without(s_copy.rest,i-1)
        return s_copy
