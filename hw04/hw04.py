...
def balanced(m):
    ...
    if is_planet(end(left(m))) and is_planet(end(right(m))):
        return check_torque_equal(m)
    
    elif is_mobile(end(left(m))) and is_mobile(end(right(m))):
        return check_torque_equal(m) and balanced(end(left(m))) and balanced(end(right(m)))
    
    elif is_mobile(end(left(m))):
        return check_torque_equal(m) and balanced(end(left(m)))
    
    elif is_mobile(end(right(m))):
        return check_torque_equal(m) and balanced(end(right(m)))
    
    elif is_planet(m):
        return True    
# if i define a function that returns the long expression, it will be simpler
